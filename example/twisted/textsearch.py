...
class TextSearcher(ztwist.ErrRaiser):
    def search (*args, **kw):
        ...
        # for testing purposes, usually would call directly
    def real_search (self, query, cb, eb):
        regexp = RegExpAttribXlate (eb).translate (query)
        print "regexp", regexp
        if regexp == None:
            return # eb has already been called
...
class TextSearchServer (ztwist.Z3950Server):
    def __init__ (self, *args, **kw):
        self.searcher = {}
        self.searcher[dbname] = TextSearcher ()
        # deliberately skip ztwist.Z3950Server.__init__

if __name__ == '__main__':
    factory = protocol.Factory ()
    factory.protocol = TextSearchServer
